...
export interface ThemeScopeState { overrides: ThemeOverrides }
...
const migrate = createSequentialMigration<ThemeScopeState>({
  1: (state) => {
    ...
    return { overrides: old.primaryColor ? { primaryColor: old.primaryColor } : {} }
  },
})

export function createThemeScopeStore(scopeId: string, initial: ThemeOverrides = {}) {
